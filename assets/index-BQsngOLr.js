(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))l(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const c of n.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&l(c)}).observe(document,{childList:!0,subtree:!0});function s(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function l(a){if(a.ep)return;a.ep=!0;const n=s(a);fetch(a.href,n)}})();function L(){const e=document.createElement("header");e.className="navbar";const t="/ecommerce-product-page";e.innerHTML=`
  ...
  `;const s=e.querySelector(".navbar__toggle"),l=e.querySelector(".navbar__close"),a=e.querySelector(".navbar__links");return s.addEventListener("click",()=>{a.classList.toggle("is-open")}),l.addEventListener("click",()=>{a.classList.remove("is-open")}),e}function $({label:e,variant:t="primary",type:s="button",icon:l=""}){const a=document.createElement("button");return a.className=`btn btn--${t}`,a.type=s,a.innerHTML=`
    ...
    `,a}const h="e-commerce-product-page";let d=k();Object.defineProperty(d,"total",{get(){return d.items.reduce((e,t)=>e+t.price*t.qty,0)}});function k(){try{return JSON.parse(localStorage.getItem(h))||{items:[]}}catch{return{items:[]}}}function f(){localStorage.setItem(h,JSON.stringify(d)),document.dispatchEvent(new CustomEvent("cart:updated",{detail:d}))}function b(e,t=1){if(!e?.slug)return;const s=d.items.find(l=>l.slug===e.slug);s?s.qty+=t:d.items.push({slug:e.slug,name:e.name,price:e.price,image:e.image,qty:t}),f(),document.dispatchEvent(new CustomEvent("cart:itemAdded",{detail:{product:e,qty:t,items:d.items}}))}function E(e){d.items=d.items.filter(t=>t.slug!==e),f()}function x(){return d}function S({slug:e,name:t,price:s,qty:l,image:a}){const n=document.createElement("div");n.className="cart-item",n.dataset.slug=e;const c=s*l;return n.innerHTML=`
...
Total: $${c.toLocaleString()}`)}}),e.querySelector(".cart-modal__checkout").appendChild(a)}return t(),document.addEventListener("cart:updated",t),e}function C(e){let t=0;const s=document.createElement("div");s.className="lightbox",s.innerHTML=`
  ...
  `;const l=s.querySelector(".lightbox__image"),a=s.querySelectorAll(".lightbox__thumb"),n=s.querySelector(".lightbox__modal"),c=s.querySelector(".lightbox__modal-image"),i=s.querySelectorAll(".lightbox__modal-thumb"),m=s.querySelector(".lightbox__close"),_=s.querySelector(".lightbox__chevron--prev"),y=s.querySelector(".lightbox__chevron--next");a.forEach((r,o)=>{r.addEventListener("click",()=>u(o)),r.addEventListener("keydown",g=>{(g.key==="Enter"||g.key===" ")&&u(o)})}),i.forEach((r,o)=>{r.addEventListener("click",()=>u(o)),r.addEventListener("keydown",g=>{(g.key==="Enter"||g.key===" ")&&u(o)})});function u(r){t=r,l.src=e[r].full,a.forEach(o=>o.classList.remove("is-active")),a[r].classList.add("is-active"),i.forEach(o=>o.classList.remove("is-active")),i[r].classList.add("is-active"),n.style.display==="flex"&&(c.src=e[r].full)}function p(){n.style.display="flex",c.src=e[t].full}l.addEventListener("click",p),l.addEventListener("keydown",r=>{(r.key==="Enter"||r.key===" ")&&p()});function v(r){let o=(t+r+e.length)%e.length;u(o)}return _.addEventListener("click",()=>v(-1)),y.addEventListener("click",()=>v(1)),document.addEventListener("keydown",r=>{n.style.display==="flex"&&(r.key==="ArrowRight"&&v(1),r.key==="ArrowLeft"&&v(-1),r.key==="Escape"&&(n.style.display="none"))}),m.addEventListener("click",()=>n.style.display="none"),n.addEventListener("click",r=>{r.target===n&&(n.style.display="none")}),s}async function w(){try{const e=await fetch("/ecommerce-product-page/data.json");if(!e.ok)throw new Error(`HTTP error! status: ${e.status}`);return await e.json()}catch(e){throw console.error("❌ Failed to fetch product data:",e),e}}document.addEventListener("DOMContentLoaded",async()=>{const e=document.querySelector("#app"),t="/ecommerce-product-page/",s=L(),l=q();e&&e.append(s,l);const a=document.createElement("div");a.id="product-container",a.className="product-container",e.appendChild(a),document.querySelector("#cart-toggle")?.addEventListener("click",()=>{document.querySelector("#cart-modal")?.classList.toggle("is-visible")});try{const c=await w();console.log("📦 Raw products data:",c),console.log("📦 Type of products:",typeof c),console.log("📦 Is products an array?",Array.isArray(c));const i=c.find(m=>m.slug==="fall-limited-sneakers");if(i){const m=C(i.images.map(_=>({thumbnail:`${t}/${_.thumbnail}`,full:`${t}/${_.full}`})));a.appendChild(m)}}catch(c){console.error("❌ Could not load Lightbox images:",c)}b({slug:"fall-sneakers",name:"Fall Limited Edition Sneakers",price:125,image:`${t}/assets/images/image-product-1-thumbnail.jpg`},3),b({slug:"summer-sneakers",name:"Summer Sneakers",price:95,image:`${t}/assets/images/image-product-1-thumbnail.jpg`},2)});
